...
const PublicProfile = () => {
  ...
  const Button = ({ loading, onClick, tooltipText, icon, color }) => (
    <button
      onClick={onClick}
      type="button"
      className={`absolute bottom-0 right-0 h-9 w-9 rounded-full border px-2 py-2 text-sm font-semibold ${color} bg-gray-800`}
      disabled={loading}
    >
      {loading ? (
  ...
  return (
    <div className="main-section bg-gray-900 text-white">
      <div className="rounded border bg-gray-800 px-6 py-6">
        ...
        <div>
          ...
          <p className="ga-2 flex items-center justify-center text-center text-gray-300">
          ...
          {interests ? (
            <div className="flex flex-wrap gap-2">
              {interests.split(",").map((interest, i) => (
                <span
                  key={i}
                  className="inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-medium bg-gray-700 text-gray-300 hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                >
                  {interest.trim()}
                </span>
              ))}
