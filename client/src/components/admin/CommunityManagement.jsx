...
const CommunityManagement = () => {
  ...
  if (!communities || !moderators) {
    return <div className="flex justify-center bg-gray-900 text-white h-screen">Loading...</div>;
  }

  return (
    <div className="flex gap-2 h-[85vh] w-full mt-3 border rounded-md bg-gray-800 text-white">
      {/* Left column */}
      <div className="flex flex-col w-full bg-gray-800 shadow-inner rounded-md border-r border-gray-600">
        <h1 className="text-lg font-bold p-4 text-center border-b-2 border-gray-600">Communities</h1>
        <div className="flex flex-col overflow-y-auto">
          {communities.map((community) => (
            <div
              key={community._id}
              className={`p-4 cursor-pointer hover:bg-gray-600 border-b border-gray-600 flex items-center ${
                selectedCommunity?._id === community._id ? "bg-gray-600" : ""
              }`}
            ...
            >
              ...
              />
              <span className="text-gray-300 text-xs md:text-base">{community.name}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Right column */}
      <div className="flex flex-col w-full bg-gray-800 rounded-md px-5 py-5 border-l border-gray-600">
        {isChangingCommunity ? (
          ...
          <>
            <h1 className="font-bold text-lg border-b border-gray-600 pb-1 mb-2">
            ...
            <div className="flex flex-col md:flex-row gap-5 mt-4">
              ...
              <div className="flex flex-col gap-2 w-full md:w-1/2">
                ...
                {selectedCommunityData.moderators?.length === 0 && (
                  <span className="text-gray-300">No moderators</span>
                )}
                  ...
                  {selectedCommunityData.moderators?.map((moderator) => (
                    <div
                      ...
                      className={`p-2 cursor-pointer border flex flex-col md:flex-row gap-2 justify-between items-center rounded ${
                        selectedModerator?._id === moderator._id ? "bg-gray-600" : ""
                      }`}
                      onClick={() => handleModeratorSelect(moderator)}
                    >
                      <span className="font-medium text-gray-300">{moderator.name}</span>
                      <button
                        disabled={isUpdating}
              ...
              <div className="flex flex-col w-full gap-2 md:w-1/2">
                ...
                <div className="flex flex-col gap-2 md:flex-row">
                  <select
                    className="bg-gray-800 border border-gray-600 text-gray-300 text-sm rounded focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                    value={newModerator}
