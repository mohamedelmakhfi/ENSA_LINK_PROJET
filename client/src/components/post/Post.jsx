...
const Post = ({ post }) => {
  ...
  return (
    <div className="border rounded bg-gray-800 p-4 m-2 hover:shadow-lg duration-300">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col">
            {userData._id === user._id ? (
              <Link to="/profile" className="font-semibold text-lg capitalize text-gray-300">
                {user.name}
              </Link>
            ) : (
              <Link
                to={`/user/${user._id}`}
                className="font-semibold text-lg capitalize text-gray-300"
              >
                {user.name}
              </Link>
            )}
      ...
      <div>
        <p
          ...
          className="my-2 cursor-pointer break-words text-gray-300"
        >
          {content}
        </p>
        <div className="mt-4">
      ...
      <div className="flex items-center justify-between mt-4">
        <div className="flex items-center gap-4">
          ...
          <button
            className="flex items-center text-lg gap-1 text-gray-300"
            onClick={() => {
              navigate(`/post/${post._id}`, {
                state: { from: location.pathname },
              });
