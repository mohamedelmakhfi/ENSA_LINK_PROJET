...
const OwnProfileCard = ({ user }) => {
  ...
  return (
    <div className="rounded-md border bg-gray-800 p-6">
      <div
        className="flex cursor-pointer justify-end text-xl text-gray-300"
      ...
      <div className="flex flex-col items-center justify-between">
        <div className="flex flex-col items-center justify-center">
          ...
          <div>
            <h2 className="mt-5 text-center text-lg font-bold text-white">{user.name}</h2>
            {user.bio ? (
      ...
      <div className="mt-4 h-20 overflow-y-auto">
        <h3 className="font-bold text-xl text-white mb-2">Interests</h3>
        {user.interests ? (
          <div className="flex flex-wrap gap-2">
            {user.interests.split(",").map((interest, i) => (
              <span
                key={i}
                className="inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-medium bg-gray-700 text-white hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
              >
                {interest.trim()}
              </span>
            ))}
      ...
      <span className="mt-3 flex flex-col items-center justify-center border-t border-gray-600">
        <Link
          className="mt-3 cursor-pointer text-primary hover:text-white hover:underline"
