...
const ReportedPost = ({ reportedPost }) => {
  ...
  return (
    <div
      className="flex items-center gap-4 cursor-pointer p-3 border-b border-gray-700"
      ...
      <div className="flex flex-col">
        <div className="flex">
          {reportedBy.slice(0, 3).map((user) => (
            <img
              key={user._id}
              className="w-8 rounded-full border-2 border-gray-700 flex-shrink-0"
              src={user.avatar}
              alt="user avatar"
            />
          ))}
          {reportedBy.length > 3 && (
            <div className="w-4 h-4 rounded-full border-2 border-gray-700 flex items-center justify-center bg-gray-300">
          ...
          <div className="ml-4">
            <span className="text-sm text-white">{reportedBy[0].name} </span>
            {reportedBy.length > 1 && (
